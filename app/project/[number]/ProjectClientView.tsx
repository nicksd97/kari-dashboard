...
export default function ProjectClientView({
  ...
  initialDeviations: DeviationRow[];
}) {
  const [project] = useState<Project | null>(initialProject);
  const [checklists] = useState<ChecklistRow[]>(initialChecklists);
  const [deviations] = useState<DeviationRow[]>(initialDeviations);
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 bg-card border-b border-border">
        <div className="mx-auto max-w-[900px] px-4 sm:px-6 py-4">
          <Link href="/" className="inline-flex items-center min-h-[44px] gap-1.5 text-[13px] font-medium mb-1 text-muted-foreground transition-colors hover:text-foreground">
          ...
          {project ? (
            <div>
              <div className="flex items-start gap-3 flex-wrap">
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-muted-foreground/70">#{project.project_number}</p>
                  <h1 className="text-[22px] sm:text-[26px] font-bold leading-tight text-foreground">{project.name}</h1>
                </div>
            ...
            <h1 className="text-[22px] font-bold text-foreground">Prosjekt #{projectNumber} ikke funnet</h1>
          )}
        </div>
      </header>

      <main className="mx-auto max-w-[900px] px-4 sm:px-6 py-6 pb-20">
        {projectFolder && <p className="text-[12px] mb-4 text-muted-foreground/70">OneDrive: {projectFolder}</p>}

        {loadingFolders ? (
          <div className="flex gap-2 mb-6">{[1, 2, 3].map((i) => <div key={i} className="h-10 w-24 rounded-lg bg-muted animate-pulse" />)}</div>
        ) : folderError && subfolders.length === 0 && !hasChecklists ? (
          <div className="rounded-xl p-6 text-center bg-card border border-border">
            <p className="text-[14px] font-medium text-foreground">{folderError}</p>
            <p className="mt-1 text-[13px] text-muted-foreground/70">Sjekk at prosjektmappen finnes under /2024/Prosjekter/ i OneDrive</p>
          </div>
        ) : (
          <>
            <div className="flex gap-1.5 overflow-x-auto pb-1 mb-5 -mx-4 px-4 sm:-mx-6 sm:px-6" style={{ scrollbarWidth: "none" }}>
              {subfolders.map((sf) => (
                <button key={sf.id} onClick={() => setActiveTab(sf.id)} className="shrink-0 rounded-lg px-4 font-medium transition-colors cursor-pointer min-h-[44px]"
                  style={{ fontSize: 13, backgroundColor: activeTab === sf.id ? "var(--foreground)" : "var(--surface)", color: activeTab === sf.id ? "var(--background)" : "var(--muted-foreground)", border: activeTab === sf.id ? "none" : "1px solid var(--border)" }}>
                  {sf.name}
                </button>
              ))}
              {hasChecklists && (
                <button onClick={() => setActiveTab("checklists")} className="shrink-0 rounded-lg px-4 font-medium transition-colors cursor-pointer min-h-[44px]"
                  style={{ fontSize: 13, backgroundColor: activeTab === "checklists" ? "var(--foreground)" : "var(--surface)", color: activeTab === "checklists" ? "var(--background)" : "var(--muted-foreground)", border: activeTab === "checklists" ? "none" : "1px solid var(--border)" }}>
                  Sjekklister ({checklists.length})
                </button>
              )}
              {hasDeviations && (
                <button onClick={() => setActiveTab("deviations")} className="shrink-0 rounded-lg px-4 font-medium transition-colors cursor-pointer min-h-[44px]"
                  style={{ fontSize: 13, backgroundColor: activeTab === "deviations" ? "var(--foreground)" : "var(--surface)", color: activeTab === "deviations" ? "var(--background)" : "var(--muted-foreground)", border: activeTab === "deviations" ? "none" : "1px solid var(--border)" }}>
...
function FolderBrowser({
  ...
  return (
    <div>
      {path.length > 1 && (
        <div className="flex items-center gap-1 mb-3 text-[12px] flex-wrap text-muted-foreground/70 min-h-[44px]">
          {path.map((p, i) => (
            <span key={p.id} className="flex items-center gap-1">
              {i > 0 && <span className="text-muted-foreground/70">/</span>}
              {i < path.length - 1 ? (
                <button onClick={() => navigateTo(i)} className="cursor-pointer hover:underline text-muted-foreground p-2 -m-2">
                  {p.name}
                </button>
              ) : (
      ...
      {loading ? (
        ...
        <div className="rounded-xl p-8 text-center bg-card border border-border">
          <p className="text-[14px] font-medium text-muted-foreground/70">Ingen filer i denne mappen</p>
        </div>
      ) : (
        <div className="rounded-xl overflow-hidden bg-card border border-border">
          {folders.map((folder, i) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 transition-colors hover:bg-muted cursor-pointer text-left"
              style={{ borderBottom: (i < folders.length - 1 || files.length > 0) ? "1px solid var(--border)" : undefined, minHeight: 52 }}
            >
              <span className="text-[20px] shrink-0">{"\uD83D\uDCC2"}</span>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-foreground">{folder.name}</p>
                <p className="text-[11px] text-muted-foreground/70">{folder.childCount} element{folder.childCount !== 1 ? "er" : ""}</p>
              </div>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="shrink-0 text-muted-foreground/70">
          ...
          <PhotoProvider>
            {files.map((file, i) => {
              ...
              const imgUrl = file.downloadUrl || `/api/graph/files/${file.id}/content`;

              return (
                <div
                  key={file.id}
                  className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-muted min-h-[52px]"
                ...
                >
                  ...
                  {pdf ? (
                    <button
                      ...
                      className="shrink-0 text-[12px] font-semibold px-3 py-2 rounded-lg cursor-pointer min-h-[44px]"
                    ...
                    <PhotoView src={imgUrl}>
                      <button className="shrink-0 text-[12px] font-semibold px-3 py-2 rounded-lg bg-secondary text-muted-foreground flex items-center cursor-pointer min-h-[44px]">
                        Vis bilde
                      </button>
                    </PhotoView>
                  ) : (
                    <a
                      href={imgUrl}
                      download={file.name}
                      className="shrink-0 text-[12px] font-semibold px-3 py-2 rounded-lg bg-secondary text-muted-foreground flex items-center min-h-[44px]"
                    >
                      Last ned
                    </a>
                  )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: "rgba(0,0,0,0.92)" }}>
      <div className="flex items-center justify-between px-4 py-3 shrink-0" style={{ backgroundColor: "rgba(0,0,0,0.95)" }}>
        <a href={downloadHref} download={file.name} className="flex items-center text-[13px] font-medium px-3 py-1.5 rounded-lg min-h-[44px]" style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "#fff" }}>
          Last ned
        </a>
        ...
        <button onClick={onClose} className="flex items-center justify-center text-[13px] font-medium px-3 py-1.5 rounded-lg cursor-pointer min-h-[44px]" style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "#fff" }}>
          Lukk
        </button>
      </div>
      ...
      {status === "error" && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <a href={downloadHref} download={file.name} className="flex items-center justify-center text-[13px] font-semibold px-4 py-2 rounded-lg min-h-[44px]" style={{ backgroundColor: "#E53935", color: "#fff" }}>
            Last ned i stedet
          </a>
        </div>
      )}
...
function ChecklistsView({ checklists }: { checklists: ChecklistRow[] }) {
  ...
  if (checklists.length === 0) {
    return (
      <div className="rounded-xl p-8 text-center bg-card border border-border">
        <p className="text-[14px] font-medium text-muted-foreground/70">Ingen sjekklister for dette prosjektet</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      {checklists.map((cl) => {
        ...
        return (
          <div key={cl.id} className="rounded-xl overflow-hidden bg-card border border-border">
            <button onClick={() => setExpandedId(isExpanded ? null : cl.id)} className="w-full flex items-center gap-3 px-4 py-3 cursor-pointer text-left" style={{ minHeight: 56 }}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-foreground">{cl.template_name}</p>
                <p className="text-[11px] text-muted-foreground/70">{cl.submitted_by && `${cl.submitted_by} \u00B7 `}{formatDate(cl.submitted_at)}</p>
              </div>
              {cl.pdf_url && (
                <a href={cl.pdf_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center justify-center shrink-0 text-[11px] font-medium px-3 py-1 rounded bg-secondary text-muted-foreground min-h-[32px]">PDF</a>
              )}
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="shrink-0 transition-transform text-muted-foreground/70" style={{ transform: isExpanded ? "rotate(180deg)" : "rotate(0deg)" }}>
            ...
            {isExpanded && items.length > 0 && (
              <div className="px-4 pb-3 border-t border-border pt-2">
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-2 py-1.5 min-h-[32px]">
                      {ok ? (
...
function DeviationsView({ deviations }: { deviations: DeviationRow[] }) {
  if (deviations.length === 0) {
    return (
      <div className="rounded-xl p-8 text-center bg-card border border-border">
        <p className="text-[14px] font-medium text-muted-foreground/70">Ingen avvik for dette prosjektet</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {open.length > 0 && (
        <p className="text-[10px] font-semibold uppercase mb-2 mt-2 text-muted-foreground/70 tracking-[0.04em]">
      ...
      {resolved.length > 0 && (
        <p className="text-[10px] font-semibold uppercase mb-2 mt-6 text-muted-foreground/70 tracking-[0.04em]">
...
function DeviationCard({ deviation: d }: { deviation: DeviationRow }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-card" style={{ border: `1px solid ${isOpen ? "#fca5a5" : "var(--border)"}` }}>
      <div className="px-4 py-3">
        <div className="flex items-start gap-2 mb-2">
          ...
          <span className="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-semibold" style={{ backgroundColor: sev.bg, color: sev.text }}>
            {sevLabel}
          </span>
        </div>

        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
          {d.reported_by_name && <span>Rapportert av: <span className="text-foreground font-medium">{d.reported_by_name}</span></span>}
          {d.responsible_name && <span>Ansvarlig: <span className="text-foreground font-medium">{d.responsible_name}</span></span>}
          <span>{formatDate(d.created_at)}</span>
        </div>

        {isOpen && deadlineText && (
          <p className="mt-2 text-[11px] font-semibold" style={{ color: deadlineText.includes("over") ? "#E53935" : "#E5A940" }}>
            Frist: {deadlineText}
          </p>
        )}

        {!isOpen && d.resolution_description && (
          <div className="mt-3 rounded-lg px-3 py-2 bg-muted/50 border border-border">
            <p className="text-[10px] font-medium mb-1 text-muted-foreground/70">Løsning</p>
            <p className="text-[12px] text-foreground">{d.resolution_description}</p>
            {d.resolved_at && <p className="mt-1 text-[10px] text-muted-foreground/70">Løst {formatDate(d.resolved_at)}</p>}
          </div>
        )}
      </div>
    </div>
